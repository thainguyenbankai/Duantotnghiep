...
export default function Home() {
    ...
    speed={800}
    loop={products.length > 5} // Chỉ bật loop khi đủ sản phẩm
    className="mySwiper !pb-12 transition-transform duration-700 ease-in-out"
>
    {(products.length > 0 ? products : Array(5).fill(null)).map((item, index) => (
        <SwiperSlide key={index}>
            {item ? (
                <div>{item.name}</div> // Sản phẩm thật
            ) : (
                <div className="bg-gray-200 h-40 rounded-lg flex items-center justify-center">
                    Loading...
                </div> // Slide giả khi chưa load
            )}
        </SwiperSlide>
    ))}
</Swiper>
