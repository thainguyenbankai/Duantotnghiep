...
export default function Account() {
  ...
  const cancelOrder = async (orderId) => {
    try {
      const token = localStorage.getItem("accessToken");
      await axios.patch(
        `https://new-server-e.onrender.com/api/order/cancel/${orderId}`,
        null,
        {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        await axios.patch(
          `https://new-server-e.onrender.com/api/auth/logout`,
          null,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const loadOrders = async () => {
    ...
    try {
      const response = await apiCall(
        "https://new-server-e.onrender.com/api/order/list-of-user",
        null,
        "get"
      );
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      const response = await apiCall(
        "https://new-server-e.onrender.com/api/user/update",
        editForm
      );
      if (response) {
        // Update localStorage
                ...
                {tabs.map((tab) =>
                  tab.key === "/wishlist" ? (
                    <Link
                      key={tab.key}
                      to={tab.key}
                      className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 w-full text-left transition-colors"
                    >
                      {renderIcon(tab.icon)}
                      <span>{tab.name}</span>
                    </Link>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 w-full text-left transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
              ...
              <>
                <div className="flex justify-between items-center mb-8">
                  ...
                  <button
                    ...
                    className={`px-8 py-2 rounded flex items-center gap-4 transition-colors ${
                      isEditing
                      ...
                      {editForm.address && (
                        <div className="p-3 bg-gray-50 rounded-lg">
                          <p className="text-sm text-gray-600">
                            Địa chỉ: <strong>{editForm.address}</strong>
                          </p>
                        </div>
                      )}
                    </div>
            ...
            {activeTab === "orders" && (
              <>
                <div className="mb-8 text-center">
                  ...
                  <p className="mt-2">
                    Quản lý và theo dõi tình trạng các đơn hàng của bạn
                  </p>
                </div>
