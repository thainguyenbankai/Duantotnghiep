...
const AddNew = () => {
    ...
    const onSubmit = (data) => {
        ...
        // Gửi field "postImages" (đồng bộ với DB Mongo)
        if (data.postImage && data.postImage.length > 0) {
            for (let i = 0; i < data.postImage.length; i++) {
                formData.append("postImages", data.postImage[i]);
            }
        }
        ...
        <div className="max-w-3xl mx-auto bg-white shadow-lg p-8 rounded-lg mt-10 overflow-y-auto h-[600px]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data" className="space-y-6">
                ...
                <div>
                    ...
                    <ReactQuill theme="snow" value={content} onChange={setContent} className="bg-white rounded-md" />
                </div>
                ...
                <div className="pt-4">
                    <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-md font-medium hover:bg-blue-700 transition duration-200">
                        ➕ Thêm bài viết
                    </button>
                </div>
            </form>
