        ...
        const fetchProduct = async () => {
            ...
            try {
                ...
                const formData = {
                    ...
                    productColors: productData.productColor?.length > 0
                        ? productData.productColor.map(color => ({ value: color }))
                        : [{ value: "#000000" }]
                };

                reset(formData); // Gọi trực tiếp reset khi có dữ liệu
            } catch (error) {
                ...
                {loadingProduct ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            ...
                            {/* Product Price */}
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">Giá sản phẩm (VNĐ) *</label>
                                <input
                                    type="number"
                            ...
                            {/* Product Category */}
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">Danh mục sản phẩm *</label>
                                {loadingCategories ? (
                                    <div className="w-full px-4 py-2 border border-gray-300 rounded-md bg-gray-50">Đang tải danh mục...</div>
                                ) : (
                                    <select
                                    ...
                                    <div className="grid grid-cols-3 gap-2 mb-4">
                                        {previewImages.map((image, index) => (
                                            <div key={index} className="relative">
                                                <img src={image} alt="" className="w-full h-20 object-cover rounded border" />
                                                <button
                                                    type="button"
                            ...
                            {/* New Images */}
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">Thêm hình ảnh mới (tùy chọn)</label>
                                <input
                                    type="file"
                                    {...register("images")}
                                    multiple
                        ...
                        <div className="space-y-4">
                            {/* Product Sizes */}
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">Kích thước sản phẩm *</label>
                                <select
                                    multiple
                            ...
                            {/* Product Colors */}
                            <div>
                                <label className="block text-gray-700 font-medium mb-2">Màu sắc sản phẩm *</label>
                            ...
                            <div>
                                ...
                                <textarea
                                    rows={6}
                                    {...register("productDescription", { required: "Mô tả sản phẩm là bắt buộc" })}
